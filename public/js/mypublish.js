			...
			success:function(result){
				if(result.status=='success'){
					...
					if(result.msg.length==0){
						var tip="<div class='zero_tip'>暂时没有发布任何房屋信息~</div>";
						$('.roombody').append(tip);
					}
					for(i=0;i<result.msg.length;i++){
						var roomlists="<div class='roomlists'></div>";
						var house_id="<div class='house_id' hidden>"+result.msg[i].id+"</div>";
						var room_image="<div class='div_img'><img src='/room"+result.msg[i].main_img+"' class='room_image'></div>";
						var room_message="<div class='room_message'></div>";
						var room_area="<div class='room_area'>"+result.msg[i].name+"</div>";
